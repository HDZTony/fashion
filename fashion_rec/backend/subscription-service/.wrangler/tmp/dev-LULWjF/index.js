...
// .wrangler/tmp/bundle-vFosM3/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_performance2();

// .wrangler/tmp/bundle-vFosM3/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_performance2();
...
__name(getPlanName, "getPlanName");
...
function getPlanTypeFromProductId(productId) {
  ...
  if (!planType) {
    throw new Error(
      `No plan type found for productId: ${productId}. Available product IDs: ${Object.keys(PRODUCT_ID_TO_PLAN_TYPE).join(", ")}.`
    );
  }
  return planType;
}
...
app.get("/plans", async (c) => {
  try {
    ...
    console.log("\u{1F4CB} /plans endpoint called:", {
      isTestMode,
    ...
    const formattedPlans = subscriptionProducts.map((p) => {
    ...
    }).filter((plan) => plan !== null).sort((a, b) => {
      const order = { member: 1 };
...
app.get("/userinfo", async (c) => {
  ...
  try {
    ...
    if (!subscriptionData || plan !== subscriptionData.plan || creemStatus !== subscriptionData.status || periodEnd !== subscriptionData.period_end || subscriptionId !== subscriptionData.subscription_id) {
...
async function updateSubscriptionFromWebhook(data, subscriptionService, creem, isTestMode, eventType) {
  ...
  try {
    ...
    if (!productId) {
      ...
      const rawStatus2 = subscription.status || eventObject?.status || "active";
      await subscriptionService.updateSubscription(
        userId,
        plan2,
        subscriptionId,
    ...
    const rawStatus = eventObject.status || data.subscription?.status || "active";
...
var createWebhookHandler = /* @__PURE__ */ __name((subscriptionService, creem, isTestMode, env2) => {
  return {
    ...
    onCheckoutCompleted: /* @__PURE__ */ __name(async (data) => {
      ...
      let userId = data.object?.metadata?.userId || data.object?.metadata?.user_id || data.checkout?.metadata?.userId || data.checkout?.metadata?.user_id || data.metadata?.userId || data.metadata?.user_id;
      if (!userId) {
        const customerEmail = data.object?.customer?.email || data.customer?.email;
        if (customerEmail) {
          console.log(`\u{1F50D} Trying to find userId by email: ${customerEmail}`);
          try {
      ...
      const productId = data.checkout?.product?.id || data.checkout?.productId || data.checkout?.items?.[0]?.productId || data.object?.product?.id || data.object?.productId || data.object?.items?.[0]?.productId || data.product?.id;
      if (!productId) {
      ...
      console.log(`\u{1F50D} Processing checkout.completed for credits purchase:`, {
        userId,
        productId,
        checkoutId: data.checkout?.id || data.object?.id
      });
...
app.post("/webhook", async (c) => {
  try {
    ...
    try {
      ...
      console.log("\u{1F4E6} Production webhook event details:", eventInfo);
    } catch (e) {
