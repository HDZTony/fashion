...
// .wrangler/tmp/bundle-UbeH3y/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_performance2();

// .wrangler/tmp/bundle-UbeH3y/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_performance2();
    ...
    onGrantAccess: /* @__PURE__ */ __name(async (context) => {
      ...
      if (userId && context.subscription?.id) {
        try {
          ...
          const subscriptionAny = subscription;
          const productId = subscriptionAny.items?.[0]?.productId || subscriptionAny.productId || subscriptionAny.product?.id || context.subscription?.items?.[0]?.productId || context.subscription?.productId;
          const plan = getPlanFromProductId(productId, isTestMode, env2);
          await subscriptionService.updateSubscription(
            userId,
            plan,
  ...
  try {
    ...
    try {
      ...
      if (Array.isArray(productsArray)) {
        const productExists = productsArray.some((p) => p?.id === body.productId);
...
app.post("/subscription/sync-from-checkout", async (c) => {
  try {
    ...
    if (subscriptionId) {
      ...
      const productId = subscriptionAny.items?.[0]?.productId || subscriptionAny.productId || subscriptionAny.product?.id || checkoutAny.productId;
      const plan = getPlanFromProductId(productId, isTestMode, env2);
      console.log("\u{1F4E6} Plan determined from product ID:", { productId, plan });
      const periodEnd = subscriptionAny.current_period_end || subscriptionAny.period_end || subscriptionAny.currentPeriodEndDate || subscriptionAny.periods && subscriptionAny.periods[0]?.end_date || null;
      console.log("\u{1F4C5} Subscription period end:", periodEnd);
