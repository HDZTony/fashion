"""
In-memory ChatKit store (MVP). Replace with a database-backed store for production.

Threads and items are lost on process restart; clients that keep an old thread_id
(e.g. Retry after error) will get a graceful ErrorEvent from FashionChatKitServer.
"""
...
class MemoryStore(Store[dict]):
    ...
    async def delete_attachment(self, attachment_id: str, context: dict) -> None:
        ...
        self._attachment_blobs.pop(attachment_id, None)

    def put_attachment_blob(self, attachment_id: str, data: bytes) -> None:
        self._attachment_blobs[attachment_id] = data

    def get_attachment_blob(self, attachment_id: str) -> bytes | None:
        return self._attachment_blobs.get(attachment_id)
